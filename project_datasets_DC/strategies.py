...
def cross_val(X,Y,splits):
	...
	for train_index, test_index in skf.split(X,Y):
	    ...
	    roc_curve_func(Y_test,pred_knn,'r','KNN',k)
	    roc_curve_func(Y_test,pred_nb,'g','NB',k)
	    roc_curve_func(Y_test,pred_dt,'b','DT',k)
	    roc_curve_func(Y_test,pred_rf,'y','RF',k)
	    k +=1

	df_accuracies = pd.DataFrame(index = np.arange(1,splits+1),columns = ['KNN','NB','DT','RF'])
	...
	df_std = pd.DataFrame(index = np.arange(1,2),columns = ['KNN','NB','DT','RF'])
	df_std.loc[len(df_std)] = [np.std(accuracy_knn),np.std(accuracy_nb),np.std(accuracy_dt),np.std(accuracy_rf)]

	return df_accuracies,df_media,df_std