...
def roc_curve_func(Y_test,predict,roc_color,label,title):

    fpr, tpr, _ = roc_curve(Y_test,predict)
    roc_auc = auc(fpr,tpr)

    # plt.figure()
    plt.plot(fpr,tpr, color=roc_color,lw=2, label= label + '(area = %0.2f)' % roc_auc)
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.title('Roc Curve width KFold = ' + str(title))
